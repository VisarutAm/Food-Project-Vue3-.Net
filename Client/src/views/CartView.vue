    ...
    <div>
      ...
      <div class="orders m-32">
        ...
        >
          <iframe
            :src="generateDriveUrl(item.driveUrl)"
            style="width: 100px; height: 100px; border: none; overflow: hidden; border-radius: 9px;"
            allow="autoplay"
            scrolling="no"
          ></iframe>
          <p class="font-bold text-lg">{{ item.name }}</p>
        ...
        <div class="cart-total m-10 w-[50%]">
          ...
          <div class="flex justify-between">
            ...
            <span class="text-gray-600 font-medium text-xl"> ฿ {{ orderStore.orderData.amount }}</span>
          </div>
          <button
  ...
  const proceedToPayment = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:5203/api/order/create-checkout-session",
